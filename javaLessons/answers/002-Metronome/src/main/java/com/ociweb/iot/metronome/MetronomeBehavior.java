...
public class MetronomeBehavior implements AnalogListener, PubSubListener, StartupListener, TimeListener {
	...
	public MetronomeBehavior(FogRuntime runtime) {
		this.tickCommandChannel = runtime.newCommandChannel(
				GreenCommandChannel.DYNAMIC_MESSAGING | FogRuntime.I2C_WRITER | FogRuntime.PIN_WRITER);
	...
	public boolean message(CharSequence topic, ChannelReader payload) {
		...
		if (requestedPBM > 0) {
			...
			if (activeBPM != requestedPBM
					&& ((System.currentTimeMillis() - requestTime) > REQ_UNCHANGED_MS || activeBPM == 0)) {
	...
	public void timeEvent(long time, int iteration) {
		if (requestedPBM != showingBPM) {
			...
			// second channel is used so we are left waiting for one cycle of the ticks
